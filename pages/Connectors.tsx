...
import { Server, Activity, AlertTriangle, Shield, ShieldCheck, RefreshCw, MoreHorizontal, Plus, Trash2, Edit2, Save, X, ChevronRight, Upload, FileText, CheckCircle2, ExternalLink, Clock, PlayCircle, Download, FileKey } from 'lucide-react';
...
const Connectors: React.FC = () => {
  ...
  const [formTab, setFormTab] = useState<'basic' | 'tech' | 'attach'>('basic');
  ...
  const handleSave = () => {
    ...
    setConnectors(prev => {
      ...
      // If activating
      if (formData.status === ConnectorStatus.PENDING_ACTIVATION) {
          // Typically activated via button below, but just in case standard save is clicked
      }

      const updatedConn = { ...formData, status: newStatus } as Connector;
  ...
  const handleActivate = () => {
     setConnectors(prev => prev.map(c => 
        c.id === formData.id ? { ...c, status: ConnectorStatus.ONLINE, certExpiryDays: 365 } as Connector : c
     ));
     alert('连接器已激活！状态变更为 Online');
     setView('list');
  };
  ...
  // Simulate Admin Actions (For Demo Purpose)
  const simulateAdminAction = (id: string, action: 'APPROVE_REG' | 'APPROVE_AUDIT') => {
    setConnectors(prev => prev.map(c => {
      ...
      if (action === 'APPROVE_AUDIT') return { ...c, status: ConnectorStatus.PENDING_ACTIVATION }; // Now moves to Pending Activation instead of Online directly
      return c;
    }));
  };
         ...
         <table className="w-full text-left border-collapse">
           <thead className="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider">
             <tr>
               ...
               <th className="px-6 py-4 font-semibold">Cert Life</th>
               ...
               <tr key={conn.id} className="hover:bg-slate-50 transition-colors">
                 <td className="px-6 py-4">
                    <div className="flex flex-col gap-2">
                      ...
                      {/* Lifecycle Simulation Buttons (DEMO ONLY) */}
                 ...
                 <td className="px-6 py-4">
                   ...
                   >
                     {conn.name}
                     {conn.status === ConnectorStatus.ONLINE && <ExternalLink size={14} className="text-slate-400" />}
                   </div>
                   {conn.status !== ConnectorStatus.PENDING_REGISTRATION && (
                 ...
                 <td className="px-6 py-4">
                   {conn.status !== ConnectorStatus.ONLINE ? (
                     <span className="text-slate-400 text-xs">-</span>
                   ) : conn.certExpiryDays < 30 ? (
                     <div className="flex items-center gap-2 text-amber-600 font-bold">
                       <AlertTriangle size={14} /> {conn.certExpiryDays} Days
                     </div>
                   ) : (
                     <div className="text-green-600 font-medium">{conn.certExpiryDays} Days</div>
                   )}
                 </td>

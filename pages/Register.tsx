...
const Register: React.FC = () => {
  ...
  const renderForm = () => (
    <div className="max-w-md mx-auto py-12 px-4 animate-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="mt-8 text-center">
        <button onClick={() => navigate('/')} className="text-slate-400 hover:text-slate-600 text-sm flex items-center justify-center gap-1 mx-auto hover:underline">
  ...
  const renderSuccess = () => (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 animate-in zoom-in duration-300">
      ...
      <button onClick={() => navigate('/')} className="px-8 py-2.5 bg-gov-700 text-white rounded-lg font-medium hover:bg-gov-800 transition-colors shadow-lg shadow-blue-500/20">
        去登录
      </button>
    </div>
  );

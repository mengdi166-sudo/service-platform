...
const Layout: React.FC<LayoutProps> = ({ children, activePage, onNavigate }) => {
  ...
  const menuItems = [
    { id: 'portal', label: '统一门户', icon: Globe },
    { id: 'identity', label: '身份管理', icon: User },
    { id: 'platform-identity', label: '平台身份', icon: Landmark },
    { id: 'connectors', label: '连接器管理', icon: Server },
    { id: 'catalog', label: '数据目录', icon: Database },
    { id: 'contracts', label: '数字合约', icon: FileSignature },
    { id: 'admin', label: '产品审核', icon: ShieldCheck },
  ];
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col shadow-xl z-20">
        <div className="p-4 border-b border-slate-700">
          <button className="flex items-center gap-3 w-full p-2 hover:bg-slate-800 rounded-lg transition-colors text-left group">
            <div className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform">
              <ShieldCheck size={24} className="text-white" />
            </div>
            <div className="flex-1 overflow-hidden">
              <h1 className="font-bold text-sm truncate">可信数据空间服务平台</h1>
              <p className="text-xs text-slate-400 truncate">Trusted Data Space</p>
            </div>
        ...
        <div className="p-4 border-t border-slate-700">
           <button 
             ...
             className="flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition-colors mb-2"
           >
              <UserPlus size={18} /> 注册新账号
           </button>
           <button 
             className="flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-red-400 hover:bg-slate-800 transition-colors"
           >
              <LogOut size={18} /> 退出登录
           </button>
        </div>
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shadow-sm z-10 flex-shrink-0">
           ...
           <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
                 ...
                 <div className="w-9 h-9 bg-slate-200 rounded-full flex items-center justify-center text-slate-500 border-2 border-white shadow-sm">
                    <User size={20} />
                 </div>
              </div>
           </div>
